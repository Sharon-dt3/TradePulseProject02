...
REST_BASE_URL = os.environ.get("POLYGON_REST_BASE_URL", "https://api.polygon.io")
STOCKS_WS_URL = os.environ.get("POLYGON_STOCKS_WS_URL", "wss://socket.polygon.io/stocks")
CRYPTO_WS_URL = os.environ.get("POLYGON_CRYPTO_WS_URL", "wss://socket.polygon.io/crypto")
...
def backfill_history(redis_client: redis.Redis, api_key: str) -> None:
    ...
    for symbol in (*STOCK_SYMBOLS, *CRYPTO_SYMBOLS.values()):
        try:
            ...
            print(f"Unable to backfill {symbol}: {error}")
...
def run_websocket(
    ...
    def on_open(ws: websocket.WebSocketApp) -> None:
        ...
        print(f"Subscribed to {', '.join(subscriptions)}.")
