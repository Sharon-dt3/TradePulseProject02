    ...
    <div className="modern-data-table overflow-x-auto">
      <table className="w-full min-w-full border-separate border-spacing-0 text-sm">
        <thead>
          <tr className="text-left text-[0.7rem] font-semibold uppercase tracking-[0.06em] text-muted">
            {columns.map((column) => {
              ...
              return <th key={column.key} scope="col" aria-sort={active ? (sort.dir === "asc" ? "ascending" : "descending") : undefined} className="whitespace-nowrap px-4 py-3 font-medium">
        ...
        <tbody>
          ...
          {!loading && sortedRows?.map((row, index) => <tr key={row[rowKey]} style={{ "--table-row-delay": `${Math.min(index * 45, 360)}ms` }} className={`modern-data-table-row ${rowClassName ? rowClassName(row) : ""}`}>{columns.map((column) => <td key={column.key} className="whitespace-nowrap px-4 py-3.5 text-fg">{column.render ? column.render(row) : (row[column.key] ?? "—")}</td>)}</tr>)}
        </tbody>
      </table>
    </div>
  );
